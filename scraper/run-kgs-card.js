/**
 * Kyrgyzstan (KGS) 스크래퍼 — 50,000 KGS 기준 (Card Payment)
 * 실행: node --env-file=.env run-kgs-card.js
 *
...
// ─── GME (API — KGS Card dm=14) ──────────────────────────────────────────────
async function scrapeGme() {
  const body = new URLSearchParams({
    method: 'GetExRate', pCurr: 'KGS', pCountryName: 'Kyrgyzstan',
    collCurr: 'KRW', deliveryMethod: '14', cAmt: '', pAmt: String(AMOUNT),
...
// ─── GMoneyTrans (API — G VM Card) ───────────────────────────────────────────
async function scrapeGmoneytrans() {
  const url = 'https://mapi.gmoneytrans.net/exratenew1/ajx_calcRate.asp'
    + `?receive_amount=${AMOUNT}`
    + '&payout_country=Kyrgyzstan'
    + '&total_collected=0'
    + `&payment_type=${encodeURIComponent('G VM Card')}`
    + '&currencyType=KGS';
  const res = await fetch(url, { signal: AbortSignal.timeout(15000) });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  const text = await res.text();
  const serviceCharge = parseField(text, 'serviceCharge') ?? 8000;
...
// ─── E9Pay (Playwright — KG_KGS > 카드송금 KGS - 방법2, remit-nation=KG04) ──
async function scrapeE9pay(browser) {
  ...
  try {
    ...
    await page.waitForSelector('#KG_KGS', { state: 'attached', timeout: 10000 });
    await page.evaluate(() => {
      const radio = document.querySelector('#KG_KGS');
      radio.checked = true;
      radio.dispatchEvent(new Event('change', { bubbles: true }));
      radio.dispatchEvent(new Event('click',  { bubbles: true }));
    });
    await page.waitForTimeout(1500);
    // Select 방법2 (KG04) from the remit-methods list
    await page.evaluate(() => {
      const a = document.querySelector('li[remit-nation="KG04"] a');
      a?.click();
    });
    await page.waitForTimeout(1500);
...
async function main() {
  ...
  console.log(`\n[${new Date().toISOString()}] Kyrgyzstan KGS Card Payment 스크래핑 시작 — run_hour: ${runHour}\n`);
  ...
  try {
    ...
    console.log(`\n✅ ${toSave.length}건 Supabase 저장 완료 (Kyrgyzstan KGS Card Payment)`);
  ...
  console.log('\n── Kyrgyzstan KGS 50,000 Card Payment 결과 요약 ─────────────────────');
