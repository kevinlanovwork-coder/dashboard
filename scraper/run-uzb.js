/**
 * Uzbekistan (USD) 스크래퍼 — 1,000 USD 기준 (Cash Pickup)
 * 실행: node --env-file=.env run-uzb.js
 *
...
// ─── GME (API — Cash Pickup dm=1) ────────────────────────────────────────────
async function scrapeGme() {
  const body = new URLSearchParams({
    method: 'GetExRate', pCurr: 'USD', pCountryName: 'Uzbekistan',
...
async function scrapeE9pay(browser) {
  ...
  try {
    ...
    await page.waitForSelector('#UZ_USD', { state: 'attached', timeout: 10000 });
    await page.evaluate(() => {
      const radio = document.querySelector('#UZ_USD');
      radio.checked = true;
...
async function scrapeHanpass() {
  const res = await fetch('https://app.hanpass.com/app/v1/remittance/get-cost', {
    ...
    body: JSON.stringify({ inputAmount: String(AMOUNT), inputCurrencyCode: 'USD',
      fromCurrencyCode: 'KRW', toCurrencyCode: 'USD', toCountryCode: 'UZ',
      memberSeq: '1', lang: 'en' }),
...
const SCRAPERS = [
  ...
  { name: 'Coinshot',    fn: () => withRetry(() => scrapeCoinshot({ country: COUNTRY, currency: 'USDUZS', amount: AMOUNT, deliveryMethod: METHOD })), needsBrowser: false },
...
async function main() {
  ...
  console.log(`\n[${new Date().toISOString()}] Uzbekistan USD 스크래핑 시작 — run_hour: ${runHour}\n`);
  ...
  try {
    ...
    console.log(`\n✅ ${toSave.length}건 Supabase 저장 완료 (Uzbekistan USD)`);
