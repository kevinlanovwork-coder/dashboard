/**
 * Canada (CAD) 스크래퍼 — 1,000 CAD 기준
 * 실행: node --env-file=.env run-cad.js
 *
...
async function scrapeGme() {
  const body = new URLSearchParams({
    method: 'GetExRate', pCurr: 'CAD', pCountryName: 'Canada',
...
async function main() {
  ...
  console.log(`\n[${new Date().toISOString()}] Canada CAD 스크래핑 시작 — run_hour: ${runHour}\n`);
  ...
  try {
    ...
    console.log(`\n✅ ${toSave.length}건 Supabase 저장 완료 (Canada CAD)`);
