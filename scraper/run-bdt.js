...
async function scrapeE9pay(browser) {
  ...
  try {
    ...
    await page.waitForSelector('#BD_BDT', { state: 'attached', timeout: 10000 });
    await page.evaluate(() => {
      const radio = document.querySelector('#BD_BDT');
      radio.checked = true;
...
async function scrapeUtransfer(browser) {
  ...
  try {
    ...
    if (!sendAmt || sendAmt === 1_000_000) throw new Error('총 송금액 계산 대기 초과');
...
async function scrapeHanpass() {
  const res = await fetch('https://app.hanpass.com/app/v1/remittance/get-cost', {
    ...
    body: JSON.stringify({ inputAmount: String(AMOUNT), inputCurrencyCode: 'BDT',
      fromCurrencyCode: 'KRW', toCurrencyCode: 'BDT', toCountryCode: 'BD',
      memberSeq: '1', lang: 'en' }),
...
async function scrapeJrf(browser) {
  ...
  try {
    ...
    await page.waitForSelector('li#BDT', { state: 'visible', timeout: 10000 });
    await page.click('li#BDT'); await page.waitForTimeout(1500);
...
async function main() {
  ...
  console.log(`\n[${new Date().toISOString()}] Bangladesh BDT 스크래핑 시작 — run_hour: ${runHour}\n`);
  ...
  try {
    ...
    console.log(`\n✅ ${toSave.length}건 Supabase 저장 완료 (Bangladesh BDT)`);
