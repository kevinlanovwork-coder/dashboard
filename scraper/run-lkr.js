...
async function scrapeGme(browser) {
  ...
  try {
    ...
    await page.fill('#CountryValue', 'Sri Lanka'); await page.waitForTimeout(300);
    await page.click('#toCurrUl li[data-countrycode="LKR"]');
...
async function scrapeSentbe(browser) {
  ...
  try {
    ...
    await page.click('.receiveAmountInput .el-select-dropdown__item:has-text("스리랑카")');
    await page.waitForTimeout(1000);
...
async function scrapeE9pay(browser) {
  ...
  try {
    ...
    await page.waitForSelector('#LK_LKR', { state: 'attached', timeout: 10000 });
    await page.evaluate(() => {
      const radio = document.querySelector('#LK_LKR');
      radio.checked = true;
...
async function scrapeCoinshot(browser) {
  ...
  try {
    ...
    await page.click('#select-receiving-currency a[data-currency="LKR"]');
    await page.waitForTimeout(1000);
...
async function scrapeHanpass() {
  const res = await fetch('https://app.hanpass.com/app/v1/remittance/get-cost', {
    ...
    body: JSON.stringify({ inputAmount: String(AMOUNT), inputCurrencyCode: 'LKR',
      fromCurrencyCode: 'KRW', toCurrencyCode: 'LKR', toCountryCode: 'LK',
      memberSeq: '1', lang: 'en' }),
...
async function main() {
  ...
  console.log(`\n[${new Date().toISOString()}] Sri Lanka LKR 스크래핑 시작 — run_hour: ${runHour}\n`);
