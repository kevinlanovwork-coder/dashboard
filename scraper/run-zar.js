/**
 * South Africa (ZAR) 스크래퍼 — 10,000 ZAR 기준
 * 실행: node --env-file=.env run-zar.js
 *
 * 지원 사업자: GME, GMoneyTrans, Hanpass
 */
...
async function scrapeGme() {
  const body = new URLSearchParams({
    method: 'GetExRate', pCurr: 'ZAR', pCountryName: 'South Africa',
...
async function scrapeHanpass() {
  const res = await fetch('https://app.hanpass.com/app/v1/remittance/get-cost', {
    ...
    body: JSON.stringify({ inputAmount: String(AMOUNT), inputCurrencyCode: 'ZAR',
      fromCurrencyCode: 'KRW', toCurrencyCode: 'ZAR', toCountryCode: 'ZA',
...
async function main() {
  ...
  console.log(`\n[${new Date().toISOString()}] South Africa ZAR 스크래핑 시작 — run_hour: ${runHour}\n`);
  ...
  try {
    ...
    console.log(`\n✅ ${toSave.length}건 Supabase 저장 완료 (South Africa ZAR)`);
