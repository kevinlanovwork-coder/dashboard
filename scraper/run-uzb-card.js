/**
 * Uzbekistan (UZS) 스크래퍼 — 1,000,000 UZS 기준 (Card Payment)
 * 실행: node --env-file=.env run-uzb-card.js
 *
 * 지원 사업자: GME, GMoneyTrans, E9Pay, Coinshot, Hanpass
 */
...
// ─── GME (API — UZCARD dm=26) ────────────────────────────────────────────────
async function scrapeGme() {
  const body = new URLSearchParams({
    method: 'GetExRate', pCurr: 'UZS', pCountryName: 'Uzbekistan',
    collCurr: 'KRW', deliveryMethod: '26', cAmt: '', pAmt: String(AMOUNT),
...
async function scrapeE9pay(browser) {
  ...
  try {
    ...
    await page.waitForSelector('#UZ_UZS', { state: 'attached', timeout: 10000 });
    await page.evaluate(() => {
      const radio = document.querySelector('#UZ_UZS');
      radio.checked = true;
...
async function scrapeCoinshot(browser) {
  ...
  try {
    ...
    await page.click('#select-receiving-currency a[data-currency="UZS"]');
    await page.waitForTimeout(1000);
    ...
    // Coinshot: fee NOT included in sendAmt, currently 0 ("CoinShot Fee included" with 0)
    const fee = 0;
...
async function scrapeHanpass() {
  const res = await fetch('https://app.hanpass.com/app/v1/remittance/get-cost', {
    ...
    body: JSON.stringify({ inputAmount: String(AMOUNT), inputCurrencyCode: 'UZS',
      fromCurrencyCode: 'KRW', toCurrencyCode: 'UZS', toCountryCode: 'UZ',
      memberSeq: '1', lang: 'en' }),
...
async function main() {
  ...
  console.log(`\n[${new Date().toISOString()}] Uzbekistan UZS Card Payment 스크래핑 시작 — run_hour: ${runHour}\n`);
  ...
  try {
    ...
    console.log(`\n✅ ${toSave.length}건 Supabase 저장 완료 (Uzbekistan UZS Card Payment)`);
  ...
  console.log('\n── Uzbekistan UZS 1,000,000 Card Payment 결과 요약 ──────────────────');
