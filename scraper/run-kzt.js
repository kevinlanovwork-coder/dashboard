/**
 * Kazakhstan (USD) 스크래퍼 — 1,000 USD 기준 (Cash Pickup)
 * 실행: node --env-file=.env run-kzt.js
 *
 * 지원 사업자: GME, GMoneyTrans, E9Pay, Coinshot, Hanpass, Cross
 */
...
const AMOUNT  = 1_000;
const METHOD  = 'Cash Pickup';

// ─── GME (API — Cash Payment dm=1) ──────────────────────────────────────────
async function scrapeGme() {
  const body = new URLSearchParams({
    method: 'GetExRate', pCurr: 'USD', pCountryName: 'Kazakhstan',
...
async function scrapeE9pay(browser) {
  ...
  try {
    ...
    await page.waitForSelector('#KZ_USD', { state: 'attached', timeout: 10000 });
    await page.evaluate(() => {
      const radio = document.querySelector('#KZ_USD');
      radio.checked = true;
      radio.dispatchEvent(new Event('change', { bubbles: true }));
      radio.dispatchEvent(new Event('click',  { bubbles: true }));
    });
    await page.waitForTimeout(2000);
    // Select 코로나페이 캐시픽업 (index 0)
    await page.evaluate(() => {
...
async function scrapeCoinshot(browser) {
  ...
  try {
    ...
    await page.click('#select-receiving-currency a[data-currency="USDKZ"]');
    await page.waitForTimeout(1000);
...
async function scrapeHanpass() {
  const res = await fetch('https://app.hanpass.com/app/v1/remittance/get-cost', {
    ...
    body: JSON.stringify({ inputAmount: String(AMOUNT), inputCurrencyCode: 'USD',
      fromCurrencyCode: 'KRW', toCurrencyCode: 'USD', toCountryCode: 'KZ',
...
// ─── Cross (Playwright — Kazakhstan USD) ─────────────────────────────────────
async function scrapeCross(browser) {
  ...
  try {
    ...
    await page.locator('div.relative:has(span:text("THB"))').click().catch(async () => {
      // Fallback: click any currency dropdown
...
async function main() {
  ...
  console.log(`\n[${new Date().toISOString()}] Kazakhstan USD 스크래핑 시작 — run_hour: ${runHour}\n`);
  ...
  try {
    ...
    console.log(`\n✅ ${toSave.length}건 Supabase 저장 완료 (Kazakhstan USD)`);
