/**
 * Kyrgyzstan (USD) 스크래퍼 — 1,000 USD 기준 (Cash Pickup)
 * 실행: node --env-file=.env run-kgs.js
 *
...
async function scrapeGme() {
  const body = new URLSearchParams({
    method: 'GetExRate', pCurr: 'USD', pCountryName: 'Kyrgyzstan',
...
async function scrapeE9pay(browser) {
  ...
  try {
    ...
    await page.waitForSelector('#KG_USD', { state: 'attached', timeout: 10000 });
    await page.evaluate(() => {
      const radio = document.querySelector('#KG_USD');
      radio.checked = true;
...
async function scrapeCoinshot(browser) {
  ...
  try {
    ...
    await page.click('#select-receiving-currency a[data-currency="USDKG"]');
    await page.waitForTimeout(1000);
...
async function scrapeHanpass() {
  const res = await fetch('https://app.hanpass.com/app/v1/remittance/get-cost', {
    ...
    body: JSON.stringify({ inputAmount: String(AMOUNT), inputCurrencyCode: 'USD',
      fromCurrencyCode: 'KRW', toCurrencyCode: 'USD', toCountryCode: 'KG',
...
async function main() {
  ...
  console.log(`\n[${new Date().toISOString()}] Kyrgyzstan USD 스크래핑 시작 — run_hour: ${runHour}\n`);
  ...
  try {
    ...
    console.log(`\n✅ ${toSave.length}건 Supabase 저장 완료 (Kyrgyzstan USD)`);
