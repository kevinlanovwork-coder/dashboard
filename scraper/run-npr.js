...
async function scrapeGme() {
  const body = new URLSearchParams({
    method: 'GetExRate', pCurr: 'NPR', pCountryName: 'Nepal',
...
async function scrapeHanpass(browser) {
  ...
  try {
    ...
    await page.fill('#countrySearch', 'Nepal'); await page.waitForTimeout(500);
    await page.locator('button[aria-label="Nepal NPR"]').first().click();
    await page.waitForTimeout(3000);
...
async function main() {
  ...
  try {
    ...
    console.log(`\n✅ ${toSave.length}건 Supabase 저장 완료 (Nepal NPR)`);
  } catch (err) {
